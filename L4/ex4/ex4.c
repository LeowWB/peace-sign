 ...
 *********************************************************/
{
    ...
    hmi.maxIdx = log2Floor(maxPartSize); // assign maxIdx to as big as possible
    hmi.minIdx = log2Floor(minPartSize);
    ...
    partInfo* current = NULL;
    int offset;
    for (offset = 0; initialSize >= maxPartSize; initialSize -= maxPartSize) {
        if (current == NULL) {
            hmi.A[hmi.maxIdx] = (current = buildPartitionInfo(offset));
        } else {
    ...
    while (initialSize > minPartSize) {
        int nextLowerLevel = log2Floor(initialSize);
        hmi.A[nextLowerLevel] = buildPartitionInfo(offset);
        int actualSize = powOf2(nextLowerLevel);
 ...
 *********************************************************/
{
    ...
    int level = log2Ceiling(wantedSize);
    if (level > hmi.maxIdx) {
        return NULL;
    }
    if (level < hmi.minIdx) {
    ...
    hmi.internalFragTotal += givenSize - wantedSize;
 ...
 *********************************************************/
{
    ...
    partInfo *prev = NULL;
    while ( current != NULL  && level != hmi.maxIdx){
